...
# Mask banks live in sibling directories under stimuli/. Two active banks:
#   masks/         -- original pure-Mondrian bank (kept for reproducibility
#                     and as a fallback if the hybrid design regresses).
#   masks_hybrid/  -- Mondrian + scrambled-face composite generated by
#                     scripts/generate_hybrid_masks.py. Active default as
#                     of 2026-04-17 after the T03_DIAG_20260417 run showed
#                     face-band leakage through the pure-Mondrian bank.
# Selection at runtime is driven by config["mask_type"] ("mondrian" or
# "hybrid"), resolved via :data:`MASK_DIR_BY_TYPE`. ``DEFAULT_MASKS_DIR``
# is kept as the legacy fallback for any caller passing ``masks_dir=None``
# without a config — ``run()`` never uses it directly.
DEFAULT_MASKS_DIR = TASK_DIR / "stimuli" / "masks"
MASK_DIR_BY_TYPE: dict[str, Path] = {
    "mondrian": TASK_DIR / "stimuli" / "masks",
    "hybrid":   TASK_DIR / "stimuli" / "masks_hybrid",
}


def resolve_masks_dir(mask_type: str) -> Path:
    """Return the stimuli directory for the given *mask_type*.

    Raises ``ValueError`` for unknown types so a typo in the YAML fails
    loudly at task startup rather than silently falling back to a bank
    the experimenter didn't intend.
    """
    try:
        return MASK_DIR_BY_TYPE[mask_type]
    except KeyError as exc:
        raise ValueError(
            f"Unknown mask_type {mask_type!r}. "
            f"Valid options: {sorted(MASK_DIR_BY_TYPE)}"
        ) from exc

# Fallback face-duration-per-frame when the monitor refresh rate can't
# be auto-detected (e.g. headless tests). 17 ms = 1 frame at 60 Hz,
# which is the stimulus-iMac refresh rate as of 2026-04-17. On 120 Hz
# hardware this gets replaced at runtime by 8 ms via
# ``win.getActualFrameRate`` in :func:`_build_psychopy_io`.
DEFAULT_FACE_FRAME_MS = 17
...
MID_TASK_BREAK_TEXT = (
    "Take a short break.\n\n"
    "Rest your eyes. The second half is about the same length.\n\n"
...
def _build_psychopy_io(
    ...
    masks_dir: Path,
    face_size_px: int,
    face_contrast: float = 1.0,
) -> tuple[TaskIO, Callable[[], None], int]:
    ...
    Also auto-detects the monitor's actual refresh rate and returns the
    corresponding frame period in ms. Caller stores this in
    ``config["face_frame_ms"]`` so the trial loop can compute SOA gaps
    against the real hardware (important for 120 Hz+ monitors — the
    original 60 Hz hardcode would silently use a 17 ms frame even when
    the display runs at 8 ms/frame, giving wrong SOAs).

    ``face_contrast`` is passed through to each face ``ImageStim``'s
    ``contrast`` attribute (1.0 = normal, 0.5 = half-signal, 0 = grey).
    The 2026-04-17 contrast pilot showed 0.5 lifts the threshold for
    high-sensitivity participants off the 17 ms floor onto a real
    psychometric curve; higher contrast is tolerable for less sensitive
    participants.
    """
    ...
    # Auto-detect refresh rate for this display. PsychoPy times 10
    # consecutive flips and returns the measured rate in Hz. If the
    # detection fails (headless, virtualised display, etc.) we fall back
    # to 60 Hz which matches the production stimulus iMac.
    refresh_hz = win.getActualFrameRate(
        nIdentical=10, nMaxFrames=120, threshold=1.0
    )
    if refresh_hz is None:
        log.warning(
            "Could not detect monitor refresh rate; falling back to "
            "%d ms/frame.",
            DEFAULT_FACE_FRAME_MS,
        )
        detected_frame_ms = DEFAULT_FACE_FRAME_MS
    else:
        detected_frame_ms = int(round(1000.0 / refresh_hz))
        log.info(
            "Detected monitor refresh: %.2f Hz (frame_ms=%d)",
            refresh_hz,
            detected_frame_ms,
        )
    ...
    # to see exactly what the participant would see. ``face_contrast``
    # is applied per-stim here rather than via a global post-draw filter
    # because ImageStim.contrast is zero-cost on the GPU.
    ...
    return io, cleanup, detected_frame_ms
...
def _run_one_trial(
    *,
    ...
    # Face is displayed for exactly one frame; the "gap" is the extra
    # gray time between face offset and mask onset needed to hit the
    # target SOA. ``face_frame_ms`` is the real frame period — detected
    # from the monitor in _build_psychopy_io, fallback to
    # DEFAULT_FACE_FRAME_MS for headless tests.
    face_frame_ms = int(config.get("face_frame_ms") or DEFAULT_FACE_FRAME_MS)
...
def run(
    ...
    *,
    ...
    if masks_dir is None:
        # Pick the bank named by config (default "hybrid"). Tests that
        # inject masks_dir explicitly continue to bypass this lookup.
        masks_dir = resolve_masks_dir(config.get("mask_type", "hybrid"))
    ...
    if io is None:
        io, cleanup, detected_frame_ms = _build_psychopy_io(
            demo=demo,
            face_ids=face_ids,
            mask_ids=mask_ids,
            faces_dir=faces_dir,
            ...
            face_contrast=float(config.get("face_contrast", 1.0)),
        )
        # Fill in face_frame_ms from the detected rate only if the caller
        # didn't pin it explicitly. Tests and headless runs pass a fixed
        # value; real sessions leave it ``None`` in the YAML and let the
        # monitor speak for itself.
        if config.get("face_frame_ms") is None:
            config["face_frame_ms"] = detected_frame_ms
    ...
    try:
        ...
        # Mid-task break (spacebar-gated). Defaults to halfway; config
        # ``break_after_main_trial`` overrides. Setting it to 0 disables
        # the break entirely (e.g. for the simulated-run test).
        raw_break = config.get("break_after_main_trial")
        if raw_break is None:
            break_after = len(trial_types) // 2
        else:
        ...
        for i, ttype in enumerate(trial_types, start=1):
            ...
            if break_after > 0 and i == break_after and i < len(trial_types):
                send_marker(outlet, "task03_break_start")
                io.show_instructions(MID_TASK_BREAK_TEXT, "space")
                send_marker(outlet, "task03_break_end")
